...
module.exports.handleEvent = function({ api, event }) {
  ...
  var tl = ["chào bạn tôi là bot của QuangSoSad ", "bạn gọi tôi có việc gì?", "tôi yêu bạn vai lon", "Yêu em <3", "Hi, chào con vợ bé:3", "Vợ gọi có việc gì không?", "Sử dụng callad để liên lạc với admin!", "Em là bot cute nhất hành tinh", "Nói gì thế con lợn", "Em đây~~~~", "Yêu anh QUANG nhất", "chị ấy là bae của Quang", "Yêu thương admin nhất", "Anh ấy là phụ trợ của admin", "Sao thế công chúa", "Đừng làm em đau ~~~~", "Rên với em nhé a á á á", "Tuyển phi công nè ạ", "Làm đĩ ko ạ? dui lắm", "Cậu cô đơn ko?", "Chịch ko ạ vã quá!!!", "Được của ló :)))", "Em dthw như chủ của em ạ", "Đừng khen em ngại quá hí hí" ,"Làm chồng em ko ạ?", "Đừng spam em nha :<<, cậu chủ em mệt lắm ời", "Cút ra", "Đừng đè em mạnh!!!", "Đút tutu thôi em đau :'(", "Yêu cậu như một cực hình\nNhấp lên nhấp xuống hai mình cùng rên", "Spam cc cút", "Yêu em ko?", "Chồng em đây rồi"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

  if ((event.body.toLowerCase() == "qutew")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: QuangSoSad để được hỗ trợ\nLink fb nè: https://www.facebook.com/xui.quang", threadID, () =>
  ...
  if ((event.body.toLowerCase() == "bot out")) {
   ...
   if ((event.body.toLowerCase() == "nayue")) {
  ...
  if ((event.body.toLowerCase() == "fwjd")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: QuangSoSad để được hỗ trợ\nLink fb nè: https://www.facebook.com/xui.quang", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
  ...
  if ((event.body.toLowerCase() == "anh quảng") || (event.body.toLowerCase() == "anh quang")) {
  ...
  if ((event.body.toLowerCase() == "a quang ngu") || (event.body.toLowerCase() == "a quang xấu trai")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  };

  if ((event.body.toLowerCase() == "quang") || (event.body.toLowerCase() == "quàng")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  };
 ...
 if ((event.body.toLowerCase() == "bsvv nha mng") || (event.body.toLowerCase() == "bsvv nha mng")) {
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin tao QuangSoSad có gì dùng lệnh -ad hoặc -adm để biết thêm chi tiết", threadID);
  };

  if ((event.body.toLowerCase() == "admin m là ai vậy bot") || (event.body.toLowerCase() == "admin m la ai vậy bot")) {
    return api.sendMessage("️Admin tao QuangSoSadcó gì dùng lệnh -ad hoặc -adm để biết thêm chi tiết", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "ông Quang") || (event.body.toLowerCase() == "ong Quang")) {
    return api.sendMessage("️Kêu chủ của tôi gì á :)?", threadID);
  };

  if ((event.body.toLowerCase() == "ôg Quang") || (event.body.toLowerCase() == "og Quang")) {
  ...
  if ((event.body.toLowerCase() == "cc") ||  (event.body.toLowerCase() == "cc")) {
   ...
   if ((event.body.toLowerCase() == "linksvv") ||  (event.body.toLowerCase() == "linksvv")) {
    return api.sendMessage("https://www.roblox.com/games/2753915549/SEA-3-Blox-Fruits?privateServerLinkCode=17904419690816820378077131343989", threadID);
  };
  if ((event.body.toLowerCase() == "gọiadmin") ||  (event.body.toLowerCase() == "gọiadmin")) {
    return api.sendMessage("@QuangSoSad", threadID);
  };
   if ((event.body.toLowerCase() == "tabthịnh") ||  (event.body.toLowerCase() == "tabthịnh")) {
    return api.sendMessage("@Thinh Chs Nak", threadID);
  };
  if ((event.body.toLowerCase() == "cuto") ||  (event.body.toLowerCase() == "cuto")) {
    return api.sendMessage("anh @QuangSoSad :>", threadID);
  };
  if ((event.body.toLowerCase() == "nhắc lần 1") ||  (event.body.toLowerCase() == "nhac lan 1")) {
    return api.sendMessage("m còn như z nữa lần 2 kick thằng :)", threadID);
  };
   if ((event.body.toLowerCase() == "tabsvv") ||  (event.body.toLowerCase() == "tabsvv")) {
    return api.sendMessage("@Đinh Vương :>", threadID);
  };
  if ((event.body.toLowerCase() == "dậy đi bọn loz") ||  (event.body.toLowerCase() == "dậy đi dcm")) {
    return api.sendMessage("cho chúng m 10s tao sẽ tab all =)))", threadID);
  };
